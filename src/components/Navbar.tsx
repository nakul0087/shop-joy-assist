...
const Navbar = ({ currentPage, onNavigate, searchQuery, onSearchChange, session }: NavbarProps) => {
    ...
    <>
      <header className="sticky top-0 z-50 bg-card border-b border-border">
        <div className="container flex items-center justify-between py-3">
          <h1 className="text-xl font-bold tracking-tight cursor-pointer" onClick={() => onNavigate("home")}>
            STYLEBAZAR
          </h1>
          ...
          <div className="flex items-center gap-3">
            <button onClick={() => onNavigate("wishlist")} className="relative p-2 rounded-full hover:bg-muted transition-colors">
              <Heart className="h-5 w-5" />
            </button>
            <button onClick={() => onNavigate("cart")} className="relative p-2 rounded-full hover:bg-muted transition-colors">
            ...
            <button onClick={() => onNavigate("profile")} className="relative p-2 rounded-full hover:bg-muted transition-colors">
              <User className="h-5 w-5" />
              {session && <span className="absolute -top-0.5 -right-0.5 h-2.5 w-2.5 rounded-full bg-success" />}
            </button>
          </div>
        </div>
